...
<body>
	<div style="width: 400px">
		<label style="display: none;" id="userId">${user.id }</label>
		<span class="title">登录账户: </span>
		<label class="label-val">${user.name }</label>
		<br />
		<span class="title">登录密码: </span>
		<label class="label-val" val="${user.password }" id="userPwd">******</label>
	</div>
    ...
    <script type="text/javascript">
    	...
    	function resetPwd(t) {
    		$.ajax({
    			type: "POST",
    			url: "home/reset_pwd?newPwd="+$(t).val()+"&id="+$("#userId").text(),
