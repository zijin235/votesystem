...
<%@page import="com.Zijin.pojo.VoteType"%>
...
<%--											<p>投票类型: ${vote.type.tips }</p>--%>
											...
											<c:if test="${fn:contains(vote.type.name(),singleType)}">
												<c:forEach items="${vote.subjectItems }" var="subjectItem">
													<p style="background-color:white;border-radius:3px;"><label class="vote-label" for="vote_item${subjectItem.id}" data-val="${subjectItem.voteNumber}" style="margin-bottom:0px;width:100%;cursor:pointer;"><input type="radio" name="vote_item${vote.id}" class="vote-item" id="vote_item${subjectItem.id}" value="${vote.id}-${subjectItem.id}" /><span class="item-title">${subjectItem.title}</span><!--<span class="item-per" style="display: inline-block;">&nbsp;</span>--></label></p>
												</c:forEach>
											</c:if>
											<c:if test="${fn:contains(vote.type.name(),muilteType)}">
												<c:forEach items="${vote.subjectItems }" var="subjectItem">
													<p style="background-color:white;border-radius:3px;"><label class="vote-label" for="vote_item${subjectItem.id}" data-val="${subjectItem.voteNumber}" style="margin-bottom:0px;width:100%;cursor:pointer;"><input type="checkbox" class="vote-item" id="vote_item${subjectItem.id}" value="${vote.id}-${subjectItem.id}" /><span class="item-title">${subjectItem.title}</span><!--<span class="item-per" style="display: inline-block;">&nbsp;</span>--></label></p>
												</c:forEach>
											</c:if>
    			...
    			success:function(data){
    				if(data.type == 'success'){
    					...
    					label.each(function(i,e){
    						...
    						$(e).children("span.item-title").text($(e).children("span.item-title").text()  +voteNum + '票');
